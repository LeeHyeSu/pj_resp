...
class ASTModel(nn.Module):
    def __init__(self, label_dim=527,
                 ...
                 input_fdim=128, input_tdim=1024, model_size='base', imagenet_pretrain=False,
                 pretrain_stage=0, load_pretrained_mdl_path=None):
        ...
        # use a pretrained models for finetuning
            ...
            audio_model = ASTModel(fstride=p_fshape, tstride=p_tshape, fshape=p_fshape, tshape=p_tshape,
                                   input_fdim=p_input_fdim, input_tdim=p_input_tdim, pretrain_stage=0, model_size=model_size)
        ...
        # use a pretrained models for additional pretraining
        elif pretrain_stage == 2:
            if fstride != fshape or tstride != tshape:
                raise ValueError('fstride != fshape or tstride != tshape, they must be same at the pretraining stage, patch split overlapping is not supported.')
            ...
            print('Additional pretraining : now load a SSL pretrained models from ' + load_pretrained_mdl_path)
            # during pretraining, fstride=fshape and tstride=tshape because no patch overlapping is used
            # here, input_fdim and input_tdim should be that used in pretraining, not that in the fine-tuning.
            # we need to know input_fdim and input_tdim to do positional embedding cut/interpolation.
            # generally it should be better to use same input_fdim during pretraining and finetuning, but input_tdim can be safely different
        ...
        # use a AST(AudioSet + ImageNet) models for additional pretraining
        elif pretrain_stage == 3:
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            
            out_dir = './pretrained_models/'
            print('now load a AST pretrained model : audioset_16_16_0.4422.pth')
            sd = torch.load(os.path.join(out_dir, 'audioset_16_16_0.4422.pth'), map_location=device)
            audio_model = ASTModel(fstride=16, tstride=16, fshape=16, tshape=16,
                                    input_fdim=128, input_tdim=1024, imagenet_pretrain=False, pretrain_stage=0, model_size='base')
            audio_model = torch.nn.DataParallel(audio_model)
            audio_model.load_state_dict(sd, strict=False)
...
if __name__ == '__main__':
    ...
    ast_mdl = ASTModel(label_dim=35,
                 ...
                 input_fdim=128, input_tdim=input_tdim, model_size='base',
                 pretrain_stage=1, load_pretrained_mdl_path='./test_mdl.pth')
