...
def train(audio_model, train_loader, test_loader, args):
    ...
    # optimizer = torch.optim.Adam([{'params': base_params, 'lr': args.lr}, {'params': mlp_params, 'lr': args.lr * args.head_lr}], weight_decay=5e-7, betas=(0.95, 0.999)) 아래 코드로 변경
    optimizer = torch.optim.Adam([{'params': base_params, 'lr': args.lr}, {'params': mlp_params, 'lr': args.lr * args.head_lr}], weight_decay=1e-6, betas=(0.95, 0.999))
    ...
    if args.loss == 'BCE':
        ...
        # weighted loss
        weights = torch.tensor([2063, 363, 1215, 501], dtype=torch.float32)
    ...
    while epoch < args.n_epochs + 1:
        ...
        # ensemble results
        cum_stats, _, _, _ = validate_ensemble(args, epoch)
...
def validate(audio_model, val_loader, args, epoch):
    ...
    with torch.no_grad():
        ...
        loss = np.mean(A_loss)
        stats, sp, se, sc = calculate_stats(audio_output, target)
